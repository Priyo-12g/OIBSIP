...
def BMI():
    ...
    if bmi <= 18.5:
        ...
        label3.config(text="Health may be at risk, \n if they do not see a doctor!")
        category = "Obesity"

    # Store the BMI data in the history list
    bmi_history.append((h, w, bmi, category))

# Function to display the BMI history
def view_history():
    history_window = Toplevel(root)
    history_window.title("BMI History")
    history_window.geometry("400x300+350+250")
    history_window.configure(bg="#f0f1f5")

    # Add labels for the columns
    Label(history_window, text="Height (cm)", font="arial 10 bold", bg="#f0f1f5").grid(row=0, column=0, padx=10, pady=10)
    Label(history_window, text="Weight (kg)", font="arial 10 bold", bg="#f0f1f5").grid(row=0, column=1, padx=10, pady=10)
    Label(history_window, text="BMI", font="arial 10 bold", bg="#f0f1f5").grid(row=0, column=2, padx=10, pady=10)
    Label(history_window, text="Category", font="arial 10 bold", bg="#f0f1f5").grid(row=0, column=3, padx=10, pady=10)

    # Add history records
    for i, record in enumerate(bmi_history):
        Label(history_window, text=record[0], font="arial 10", bg="#f0f1f5").grid(row=i + 1, column=0)
        Label(history_window, text=record[1], font="arial 10", bg="#f0f1f5").grid(row=i + 1, column=1)
        Label(history_window, text=record[2], font="arial 10", bg="#f0f1f5").grid(row=i + 1, column=2)
        Label(history_window, text=record[3], font="arial 10", bg="#f0f1f5").grid(row=i + 1, column=3)
...
Button(root, text="History", width=15, height=2, font="arial 10 bold", bg="#1f6e68", fg="white", command=view_history).place(x=280, y=380)
