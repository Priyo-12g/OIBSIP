...
class Weather(Tk):
    def __init__(self):
        ...
        self.unit_var = StringVar(value="Celsius")  # Default unit is Celsius
        self.original_temp_k = None
        
    def __gui(self):
       #get some image
       self.back_img = PhotoImage(file="Images/Clear.png")
       self.back_img2 = PhotoImage(file="gradient_wallpaper_1.gif")

       #background_image
       back_label = Label(self,image=self.back_img)
       ...
       # Add a dropdown menu for unit selection next to the search box
       self.unit_menu = ttk.Combobox(self, textvariable=self.unit_var, values=["Celsius", "Fahrenheit"], state="readonly", font=("Cambria", 12, 'bold'))
       self.unit_menu.place(x=340, y=25)  
       self.unit_menu.bind("<<ComboboxSelected>>", self.update_temperature_unit)
    ...
    def __set_information(self,weather):
        ...
        if weather['cod']=='404' and weather['message']=='city not found':
            ...
            # Store the original temperature in Kelvin
            self.original_temp_k = weather['main']['temp']

            # Set temperature to Celsius initially
    ...
    def update_temperature_display(self):
        ...
        if self.original_temp_k is None:
            return

        if self.unit_var.get() == "Celsius":
            temp_celsius = int(self.original_temp_k - 273.15)
            self.temperature['text'] = temp_celsius
            self.degree['text'] = "°C"
        elif self.unit_var.get() == "Fahrenheit":
            temp_fahrenheit = int((self.original_temp_k - 273.15) * 9/5 + 32)
            self.temperature['text'] = temp_fahrenheit
            self.degree['text'] = "°F"

    def update_temperature_unit(self, event):
